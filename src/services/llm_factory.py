...
class LLMRouter:
    ...
    def __init__(self, factory: Optional[LLMFactory] = None, config: Optional[Dict[str, Any]] = None):
        """
        ...
        self.fallback_providers = self.config.get("fallback_providers", ["openai", "anthropic", "google"])
    ...
    async def generate(
        ...
        try:
            ...
            if not self.enable_fallback:
                raise LLMError(f"Provider {primary_provider} failed: {e}", provider=primary_provider, model=request.model)
        ...
        for provider in fallback_providers:
            try:
                ...
                if not model_supported:
                    ...
                    logger.info(f"Model {original_model} not available in {provider}, using {request.model}")
