...
from .enums import (
    CommandWord, SubjectContentReference, TopicName, 
    CalculatorPolicy, Tier, CognitiveLevel, QuestionOrigin,
    LLMProvider, LLMModel, QualityAction
)
...
class SolverStep(BaseModel):
    ...
    mathematical_expression_latex: Optional[str] = Field(None, description="Math formula/calculation")
...
class QuestionTaxonomy(BaseModel):
    ...
    subject_content_references: List[SubjectContentReference] = Field(..., description="Syllabus codes")
    ...
    difficulty_estimate_0_to_1: Optional[float] = Field(None, ge=0, le=1, description="Difficulty rating")
...
class AssetRecreationData(BaseModel):
    ...
    relationships: List[Dict[str, Any]] = Field(default_factory=list, description="Element relationships")
...
class QuestionAsset(BaseModel):
    ...
    recreation_data: Optional[AssetRecreationData] = Field(None, description="Manim recreation data")
...
class GenerationRequest(BaseModel):
    ...
    subject_content_refs: Optional[List[SubjectContentReference]] = Field(None, description="Specific syllabus refs")
...
class QualityDecision(BaseModel):
    ...
    mathematical_accuracy: Optional[float] = Field(None, ge=0, le=1, description="Math accuracy score")
    cambridge_compliance: Optional[float] = Field(None, ge=0, le=1, description="Syllabus compliance")
    grade_appropriateness: Optional[float] = Field(None, ge=0, le=1, description="Grade level match")
...
class GenerationSession(BaseModel):
    ...
    quality_decisions: List[QualityDecision] = Field(default_factory=list, description="Quality assessments")
    agent_results: List[AgentResult] = Field(default_factory=list, description="All agent results")
    status: GenerationStatus = Field(default=GenerationStatus.CANDIDATE, description="Session status")
...
def validate_question_structure(question: Question) -> bool:
    ...
    criteria_total = sum(c.marks_value for c in question.solution_and_marking_scheme.mark_allocation_criteria)
