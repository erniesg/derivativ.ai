...
from ..models.question_models import (
    SolutionAndMarkingScheme, MarkingCriterion, FinalAnswer, 
    GenerationRequest, AgentResult, MarkType
)
...
class MarkerAgent(BaseAgent):
    ...
    async def _execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self._observe(f"Parsed marking request for question: {question['question_text'][:50]}...")
            ...
            self._observe(f"Marking generation failed: {str(e)}")
            raise MarkerAgentError(f"Marking scheme generation failed: {e}")
    
    def _parse_marking_request(self, input_data: Dict[str, Any]) -> Tuple[Dict[str, Any], GenerationRequest]:
    ...
    async def _generate_marking_scheme_with_retries(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                marking_scheme = await self._generate_marking_scheme_primary(question, config, attempt)
                ...
                if attempt == max_retries and self.agent_config.get("enable_fallback"):
                    try:
                        ...
                        marking_scheme = await self._generate_marking_scheme_fallback(question, config)
                        if marking_scheme:
...
Return JSON with: total_marks, mark_allocation_criteria (with criterion_text, marks_value, mark_type), final_answers"""
        ...
        try:
            ...
            for answer_data in answers_data:
                if isinstance(answer_data, dict):
                    final_answers.append(FinalAnswer(
            ...
            self._act(f"Converted to SolutionAndMarkingScheme: {len(mark_criteria)} criteria, {total_marks} marks")
            return marking_scheme
    ...
    async def _validate_generated_marking_scheme(
        ...
        try:
            ...
            allocated_marks = sum(criterion.marks_value for criterion in marking_scheme.mark_allocation_criteria)
            if allocated_marks != marking_scheme.total_marks_for_part:
                logger.warning(f"Mark allocation mismatch: criteria={allocated_marks}, total={marking_scheme.total_marks_for_part}")
            
            # Validate mark types
            mark_types = [criterion.mark_type_primary for criterion in marking_scheme.mark_allocation_criteria]
            if not any(mark_type in [MarkType.M, MarkType.A, MarkType.B] for mark_type in mark_types):
