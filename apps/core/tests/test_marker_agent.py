...
from src.models.question_models import (
    ...
    GenerationRequest, Question, AgentResult, MarkType
)
from src.models.enums import LLMModel, CalculatorPolicy
from src.services.llm_service import LLMResponse, LLMError, LLMTimeoutError
...
class TestMarkerAgent:
    ...
    async def test_successful_marking_scheme_generation_end_to_end(
        ...
        sample_question, sample_marking_scheme_json, sample_generation_config
    ):
        ...
        import json
        mock_llm_service.generate = AsyncMock(return_value=LLMResponse(
        ...
        input_data = {
            ...
            "config": sample_generation_config.model_dump()
        }
    ...
    @pytest.mark.asyncio
    async def test_mark_type_assignment_logic(
        self, sample_marking_scheme_json
    ):
    ...
    @pytest.mark.asyncio
    async def test_marking_scheme_object_conversion_from_json(
        self, sample_marking_scheme_json
    ):
    ...
    async def test_agent_reasoning_steps_logged(
        self, prompt_manager, json_parser,
        sample_question, sample_generation_config, sample_marking_scheme_json
    ):
    ...
    async def test_integration_with_mock_services(
        ...
        marking_scheme_json = {
            ...
            "mark_allocation_criteria": [
                ...
                {"criterion_text": "Correct substitution r = 5", "marks_value": 1, "mark_type": "M"},
...
class TestMarkerAgentPerformance:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_marking_generation_handling(
        self, prompt_manager, json_parser
    ):
