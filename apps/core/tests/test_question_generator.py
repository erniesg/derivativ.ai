...
from src.models.question_models import GenerationRequest, Question, AgentResult
from src.models.enums import LLMModel, CommandWord, SubjectContentReference
...
class TestQuestionGeneratorAgent:
    ...
    async def test_successful_question_generation_end_to_end(
        ...
        sample_generation_request, sample_question_json
    ):
    ...
    @pytest.mark.asyncio
    async def test_question_validation_failure(
        self, mock_llm_service, prompt_manager, json_parser
    ):
        ...
        assert "failed" in result.error.lower()  # Question validation failure leads to retry exhaustion
    ...
    @pytest.mark.asyncio
    async def test_question_object_conversion_fallback_mode(
        self, sample_generation_request
    ):
        ...
        assert len(question.solution_and_marking_scheme.mark_allocation_criteria) == 1  # Default criterion
