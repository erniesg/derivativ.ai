...
class HTTPRequestExecutor:
    ...
    async def _simulate_http_request(  # noqa: PLR0911
        self, url: str, method: str, headers: dict[str, str], body: Optional[str], timeout_ms: int
    ) -> MockResponse:
        ...
        if "/api/test/success" in url:
            return MockResponse(
                status_code=200, data={"status": "success", "message": "Request completed"}
            )

        elif "/api/test/created" in url:
            return MockResponse(status_code=201, data={"id": "12345", "status": "created"})

        elif "/api/test/server_error" in url:
            return MockResponse(status_code=500, text_content="Internal Server Error")

        elif "/api/test/not_found" in url:
            return MockResponse(status_code=404, text_content="Not Found")
            ...
            if attempt_count < 3:
                ...
                return MockResponse(
                    status_code=200, data={"status": "success", "attempts": attempt_count}
                )
            ...
            return MockResponse(
                status_code=200, data={"status": "authenticated", "user": "test_user"}
            )

        else:
            # Default success response
            return MockResponse(
                status_code=200, data={"url": url, "method": method, "received": True}
            )
...
class TestHTTPRequestExecution:
    ...
    async def test_created_response(self, executor):
        ...
        response = await executor.execute_request(
            url="https://api.example.com/api/test/created", method="POST"
        )
    ...
    async def test_retry_then_succeed(self, executor):
        ...
        retry_requests = [r for r in executor.request_history if "retry_then_succeed" in r["url"]]
    ...
    async def test_different_http_methods(self, executor):
        ...
        for method in methods:
            response = await executor.execute_request(
                url="https://api.example.com/api/test/success", method=method
            )
    ...
    async def test_custom_headers(self, executor):
        ...
        response = await executor.execute_request(
            url="https://api.example.com/api/test/success", headers=custom_headers
        )
...
class TestAuthenticationManager:
    ...
    def test_custom_authentication(self):
        ...
        auth_config = {
            "type": "custom",
            "custom_headers": {"X-Custom-Auth": "custom-value", "X-Client-ID": "client-12345"},
        }
    ...
    async def test_auth_integration_with_executor(self):
        ...
        response = await executor.execute_request(
            url="https://api.example.com/api/test/auth_required", headers=headers
        )
    ...
    async def test_auth_failure_without_credentials(self):
        ...
        with pytest.raises(Exception) as exc_info:
            await executor.execute_request(url="https://api.example.com/api/test/auth_required")
...
class TestTemplateRenderer:
    ...
    def test_nested_variable_access(self):
        ...
        context = {
            "user": {"id": "12345", "profile": {"name": "Alice", "email": "alice@example.com"}}
        }
    ...
    def test_complex_object_substitution(self):
        ...
        context = {"config": {"debug": True, "timeout": 30}, "items": ["item1", "item2", "item3"]}
    ...
    def test_derivativ_payload_template(self):
        ...
        context = {
            ...
            "steps": {"previous_step": {"result": {"generated_count": 5, "avg_quality": 0.82}}},
        }
...
class TestResponseProcessing:
    ...
    async def test_json_response_processing(self):
        ...
        response = await executor.execute_request(url="https://api.example.com/api/test/success")
...
if __name__ == "__main__":
    ...
    async def test_integration():
        ...
        try:
            ...
            print(
                f"✅ Retry mechanism test completed (expected failure): {len(executor.request_history)} attempts made"
            )
