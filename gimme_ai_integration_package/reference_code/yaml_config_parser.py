...
class YAMLConfigParser:
    ...
    def parse_yaml_file(
        self, config_path: str, template_variables: Optional[dict[str, Any]] = None
    ...
    def parse_yaml_string(
        self, yaml_content: str, template_variables: Optional[dict[str, Any]] = None
    ) -> ParsedWorkflowConfig:
    ...
    def _validate_cron_field(self, field: str, min_val: int, max_val: int) -> bool:  # noqa: PLR0911
    ...
    def _validate_retry_config(self, retry: dict[str, Any], step_index: int) -> None:
        ...
        if "limit" not in retry:
            ...
            self.validation_errors.append(
                f"Step {step_index} retry limit must be a non-negative integer"
            )
        ...
        if "delay" not in retry:
            ...
            self.validation_errors.append(
                f"Step {step_index} retry delay must be valid duration (e.g., '5s', '1m')"
            )
        ...
        if "backoff" in retry:
            ...
            if retry["backoff"] not in valid_backoff:
                self.validation_errors.append(
                    f"Step {step_index} invalid backoff strategy: {retry['backoff']}"
                )
    ...
    def _validate_monitoring_config(self, monitoring: dict[str, Any]) -> None:
        ...
        if "webhook_url" in monitoring:
            ...
            if not isinstance(webhook_url, str) or not (
                webhook_url.startswith("http://") or webhook_url.startswith("https://")
            ):
                self.validation_errors.append(
                    "Monitoring webhook_url must be a valid HTTP/HTTPS URL"
                )
