...
class ExecutionPlanner:
    ...
    def build_execution_plan(self, steps: list[StepConfig]) -> ExecutionPlan:
        ...
        return ExecutionPlan(phases=phases, total_steps=len(steps))
    ...
    def _build_execution_phases(self) -> list[ExecutionPhase]:
        ...
        while remaining:
            ...
            for group_name, group_steps in grouped_steps.items():
                ...
                ready_group_steps = [
                    s for s in group_steps if s.name in [rs.name for rs in ready_steps]
                ]
...
class WorkflowExecutor:
    ...
    async def execute_plan(self, plan: ExecutionPlan) -> dict[str, StepResult]:
        ...
        print(
            f"Executing workflow plan with {len(plan.phases)} phases, {plan.total_steps} total steps"
        )
    ...
    async def _execute_phase(self, phase: ExecutionPhase, phase_index: int) -> None:
        ...
        print(
            f"Phase {phase_index}: {len(phase.sequential_steps)} sequential steps, {len(phase.parallel_groups)} parallel groups"
        )
...
class MockAPIExecutor:
    ...
    async def call_api(
        ...
        if "questions/generate" in endpoint:
            ...
            return {"document_id": f"doc_{step_name}_{self.call_count}", "status": "generated"}
        elif "documents/store" in endpoint:
            return {"storage_id": f"storage_{self.call_count}", "status": "stored"}
        elif "test/error" in endpoint:
            raise Exception(f"Simulated error for {step_name}")
        else:
            return {"result": f"success_{step_name}", "call_id": self.call_count}
...
class TestExecutionPlanning:
    ...
    def test_dependency_based_phases(self):
        ...
        steps = [
            ...
            StepConfig(
                name="cleanup", endpoint="/api/cleanup", depends_on=["process1", "process2"]
            ),
        ]
    ...
    def test_parallel_group_with_dependencies(self):
        ...
        steps = [
            StepConfig(name="init", endpoint="/api/init"),
            StepConfig(
                name="parallel1", endpoint="/api/p1", parallel_group="group1", depends_on=["init"]
            ),
            StepConfig(
                name="parallel2", endpoint="/api/p2", parallel_group="group1", depends_on=["init"]
            ),
            StepConfig(
                name="parallel3", endpoint="/api/p3", parallel_group="group1", depends_on=["init"]
            ),
        ]
    ...
    def test_missing_dependency(self):
        ...
        steps = [StepConfig(name="step1", endpoint="/api/step1", depends_on=["nonexistent"])]
...
class TestWorkflowExecution:
    ...
    async def test_concurrency_control(self):
        ...
        steps = [
            StepConfig(
                name="task1", endpoint="/api/task", parallel_group="limited", max_parallel=2
            ),
            StepConfig(
                name="task2", endpoint="/api/task", parallel_group="limited", max_parallel=2
            ),
            StepConfig(
                name="task3", endpoint="/api/task", parallel_group="limited", max_parallel=2
            ),
            StepConfig(
                name="task4", endpoint="/api/task", parallel_group="limited", max_parallel=2
            ),
        ]
    ...
    async def test_error_handling_continue_on_error(self):
        ...
        assert (
            results["error_step"].status == "skipped"
        )  # Should be skipped due to continue_on_error
...
if __name__ == "__main__":
    ...
    async def test_integration():
        ...
        print(
            f"✅ Parallel plan: {len(parallel_plan.phases)} phases, {len(parallel_plan.phases[0].parallel_groups)} groups"
        )

        # Test 3: Complex Derivativ workflow
        derivativ_steps = [
            StepConfig(
                name="generate_algebra", endpoint="/api/questions/generate", parallel_group="gen"
            ),
            StepConfig(
                name="generate_geometry", endpoint="/api/questions/generate", parallel_group="gen"
            ),
            StepConfig(
                name="create_worksheet",
                endpoint="/api/docs/generate",
                depends_on=["generate_algebra", "generate_geometry"],
            ),
            StepConfig(
                name="store_docs", endpoint="/api/docs/store", depends_on=["create_worksheet"]
            ),
        ]
