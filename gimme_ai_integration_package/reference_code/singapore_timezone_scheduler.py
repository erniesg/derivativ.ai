...
class SingaporeTimezoneScheduler:
    ...
    def parse_cron_schedule(
        self, cron_string: str, timezone: str = "UTC", description: str = ""
    ) -> CronSchedule:
    ...
    def convert_to_utc(
        self, schedule: Union[str, CronSchedule], source_timezone: str = "Asia/Singapore"
        ...
        converted_day = self._convert_day_field(schedule.day, day_adjustment, conversion_notes)
    ...
    def generate_cloudflare_wrangler_config(
        self, schedules: list[Union[str, CronSchedule]], source_timezone: str = "Asia/Singapore"
    ) -> dict[str, Any]:
        ...
        for schedule in schedules:
            ...
            conversion_comments.append(f"# {original_desc} ({source_timezone}) -> UTC")

        return {
            "triggers": {"crons": cron_entries},
            "conversion_info": {
    ...
    def _convert_hour_field(
        self, hour_field: str, offset_hours: float, notes: list[str]
    ) -> tuple[str, int]:
    ...
    def _convert_day_field(self, day_field: str, day_adjustment: int, notes: list[str]) -> str:
        ...
        if day_adjustment != 0 and day_field != "*":
            notes.append(
                f"Day field '{day_field}' may need manual adjustment due to timezone conversion"
            )

        return day_field

    def _convert_weekday_field(  # noqa: PLR0911
        self, weekday_field: str, day_adjustment: int, notes: list[str]
    ) -> str:
        ...
        if day_adjustment == -1:
            ...
            if "," in weekday_field:
                ...
                notes.append(
                    f"Weekday field '{weekday_field}' may need manual adjustment (moved to previous day)"
                )
                return weekday_field
            elif "-" in weekday_field:
                # Range case - warn user
                notes.append(
                    f"Weekday range '{weekday_field}' may need manual adjustment (moved to previous day)"
                )
                return weekday_field
            else:
                # Single weekday
                try:
                    ...
                    notes.append(
                        f"Weekday {weekday_field} adjusted to {adjusted_weekday} (previous day)"
                    )
                    return str(adjusted_weekday)
                except ValueError:
                    notes.append(
                        f"Weekday field '{weekday_field}' could not be adjusted automatically"
                    )
                    return weekday_field
            ...
            if "," in weekday_field:
                notes.append(
                    f"Weekday field '{weekday_field}' may need manual adjustment (moved to next day)"
                )
                return weekday_field
            elif "-" in weekday_field:
                notes.append(
                    f"Weekday range '{weekday_field}' may need manual adjustment (moved to next day)"
                )
                return weekday_field
            else:
                try:
                    ...
                    adjusted_weekday = (weekday + 1) % 7
                    notes.append(
                        f"Weekday {weekday_field} adjusted to {adjusted_weekday} (next day)"
                    )
                    return str(adjusted_weekday)
                except ValueError:
                    notes.append(
    ...
    def _calculate_next_execution(
        self, local_schedule: CronSchedule, utc_schedule: CronSchedule
        ...
        try:
            ...
            if (
                local_schedule.minute.isdigit()
                and local_schedule.hour.isdigit()
                and local_schedule.day == "*"
                and local_schedule.month == "*"
                and local_schedule.weekday == "*"
            ):
...
class DerivativScheduleTemplates:
    ...
    def get_daily_question_generation_schedule() -> dict[str, Any]:
        ...
        return {
            ...
            "next_execution_sgt": result.next_execution_local.isoformat()
            if result.next_execution_local
            else None,
            "next_execution_utc": result.next_execution_utc.isoformat()
            if result.next_execution_utc
            else None,
        }
