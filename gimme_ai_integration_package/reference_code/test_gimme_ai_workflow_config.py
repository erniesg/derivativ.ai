...
class WorkflowConfigValidator:
    ...
    def _validate_cron_schedule(schedule: str) -> list[str]:
        ...
        if len(parts) != 5:
            errors.append(
                "Cron schedule must have exactly 5 fields (minute hour day month weekday)"
            )
            return errors
    ...
    @staticmethod
    def _validate_cron_field(field: str, min_val: int, max_val: int) -> bool:  # noqa: PLR0911
        ...
        if "-" in field:
            try:
                ...
                end_num = int(end)
                return (
                    min_val <= start_num <= max_val
                    and min_val <= end_num <= max_val
    ...
    def _validate_timezone(timezone: str) -> list[str]:
        ...
        if timezone not in valid_timezones:
            errors.append(
                f"Unsupported timezone: {timezone}. Supported: {', '.join(valid_timezones)}"
            )
    ...
    def _validate_auth_config(auth: dict[str, Any]) -> list[str]:
        ...
        if "type" not in auth:
            ...
            if auth["type"] not in valid_types:
                errors.append(
                    f"Invalid auth type: {auth['type']}. Valid types: {', '.join(valid_types)}"
                )
    ...
    def _validate_steps(steps: list[dict[str, Any]]) -> list[str]:
        ...
        for i, step in enumerate(steps):
            ...
            # Validate endpoint
            if "endpoint" in step and (
                not isinstance(step["endpoint"], str) or not step["endpoint"].strip()
            ):
    ...
    def _validate_retry_config(retry: dict[str, Any], step_index: int) -> list[str]:
        ...
        if "delay" not in retry:
            ...
            errors.append(
                f"Step {step_index} retry delay must be a valid duration (e.g., '5s', '1m')"
            )
...
class TestWorkflowConfigValidation:
    ...
    def test_valid_complete_config(self):
        ...
        config = {
            ...
            "auth": {"type": "bearer", "token": "${DERIVATIV_API_KEY}"},
            "variables": {"topics": ["algebra", "geometry"], "questions_per_topic": 8},
            "steps": [
                {
                    ...
                    "retry": {"limit": 3, "delay": "10s", "backoff": "exponential"},
                    "payload_template": "{}",
                    "timeout": "5m",
                },
                {
                    ...
                    "retry": {"limit": 2, "delay": "5s", "backoff": "linear"},
                },
            ],
        }
    ...
    def test_missing_required_fields(self):
        ...
        config = {"description": "Missing name and api_base"}
    ...
    def test_valid_cron_schedules(self):
        ...
        for schedule in valid_schedules:
            ...
            assert (
                schedule_errors == []
            ), f"Valid schedule '{schedule}' failed validation: {schedule_errors}"
    ...
    def test_empty_steps(self):
        ...
        config = {"name": "test", "api_base": "https://api.example.com", "steps": []}
    ...
    def test_invalid_step_dependencies(self):
        ...
        config = {
            "name": "test",
            "api_base": "https://api.example.com",
            "steps": [{"name": "step1", "endpoint": "/test1", "depends_on": ["nonexistent_step"]}],
        }
...
class TestRetryConfiguration:
    ...
    def test_valid_retry_configs(self):
        ...
        valid_configs = [
            ...
            {"limit": 1, "delay": "1m", "backoff": "linear"},
            {
                "limit": 0,  # No retries
                "delay": "10s",
                "backoff": "constant",
    ...
    def test_duration_string_validation(self):
        ...
        for duration in valid_durations:
            assert WorkflowConfigValidator._validate_duration_string(
                duration
            ), f"Valid duration '{duration}' failed validation"

        for duration in invalid_durations:
            assert not WorkflowConfigValidator._validate_duration_string(
                duration
            ), f"Invalid duration '{duration}' passed validation"
